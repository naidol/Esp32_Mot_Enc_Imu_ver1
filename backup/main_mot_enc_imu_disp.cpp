...
sensor_msgs__msg__Imu imu_ros_msg;
...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
    ...
    bno.getEvent(&event);  // this reads orientation XYZ | angular vel XYZ | linear accel XYZ
    ...
    imu_msg.x = event.orientation.x;
    imu_msg.y = event.orientation.y;
    imu_msg.z = event.orientation.z;
    ...
    display.print("Orientation X: "); display.println(event.orientation.x);
    display.print("Orientation Y: "); display.println(event.orientation.y);
    display.print("Orientation Z: "); display.println(event.orientation.z);
